...
function fixStm(n, parentScript) {
  ...
  case IF:
    ...
    if (n.thenPart === undefined) print("fixst " + n.lineno + n);
...
function walkASTgenerator() {
  ...
  function recur(n) {
    if (jt[n.type] === undefined) print("wag " + n.type + " " + n.lineno);
    return jt[n.type].apply(o, arguments);
  }
...
// The type-inferrence pass stores its results in infTypes.
var infTypes;
...
// Aobj, number -> string
function objToType(o, addr) {

  // function node -> string
  function funToType(n) {
    // FIXME: tag built-in nodes w/ their types
    if (n.builtin) return "function";
    var addr = n.addr;

  }

  function arrayToType() {
    var elmtype = BOTTOM;
    o.forEachOwnProp(function(p) {
      if (propIsNumeric(p)) elmtype = avjoin(elmtype, o.getOwnExactProp(p));
    });
    elmtype = avalToType(elmtype);
    return (/\|/.test(elmtype) || elmtype === "any") ?
      "Array" : "Array[" + elmtype + "]";
  }

  // Aval -> string
  function avalToType(av) {
    var i = 1, base = mergeBoolsInBase(av.base), types = [];
    ...
    av.forEachObj(function (o) {
      types.push(objToType(o)); // TODO: continue here, must know addr
  ...
  if (infTypes[addr] !== undefined) return t || "any";
  infTypes[addr] = ""; // Marks that the object has been visited
  if (o["-fun"]) return infTypes[addr] = funToType(o["-fun"]);

  var c = o.getProp("constructor-");
  var types = [];
  if (c === undefined) 
    return infTypes[addr] = "Global Object";
  c.forEachObj(function(o) {if (o["-fun"]) types.push(o["-fun"].fname.name);});
  if (types.length === 0)
    throw errorWithCode(CFA_ERROR, "Didn't find a name for constructor");
  normalizeUnionType(types);
  if (types.length === 1)
    return infTypes[addr] = (types[0] === "Array") ? arrayToType() : types[0];
  return infTypes[addr] = "<" + types.join(" | ") + ">";
};
...
function attachMethod(o, mname, mcode, arity) {
  new Aobj({ addr: newCount(),
             proto: function_prototype_av,
             fun: funToNode(mname, mcode, arity) });
  o.addProp(mname + "-",  { aval: makeObjAval(count), enum : false });
}
...
function initCoreObjs() {
  ...
  var o = new Aobj({addr: newCount(),
                    proto: o_pav,
                    fun: funToNode("Object", _Object)});
  var oav = makeObjAval(count);
  ...
  (function () {
    ...
    var a = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun: funToNode("Array", _Array) });
    var aav = makeObjAval(count);
  ...
  (function () {
    ...
    var n = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun: funToNode("Number", _Number) });
    var nav = makeObjAval(count);
  ...
  (function () {
    ...
    var s = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun: funToNode("String", _String) });
    var sav = makeObjAval(count);
  ...
  (function () {
    ...
    var e = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun: funToNode("Error", _Error) });
    var eav = makeObjAval(count);
  ...
  (function () {
    ...
    var r = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun: funToNode("RegExp", _RegExp) });
    var rav = makeObjAval(count);
  ...
  (function () {
    ...
    var d = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun: funToNode("Date", _Date) });
    var dav = makeObjAval(count);
  ...
  (function () {
    ...
    var b = new Aobj({ addr: newCount(),
                       proto: o_pav,
                       fun : funToNode("Boolean", _Boolean) });
    var bav = makeObjAval(count);
...
(function() {
  ...
  override(HOOK, function(n, fr) {
    ...
    if (!test.isTruthy()) {
      print("dbg 24 " + n.lineno + " " + ch[2].type);
      try {
        ans = evalExp(ch[2], ans.fr);
      }
      catch (e) {
        if (!(e instanceof StackCleaner)) console.trace();
        throw e;
      }
      print("dbg 27");
      av = avjoin(av, ans.v);
  ...
  override(CALL, function(n, fr) {
    ...
    var ans, ans1, errval, rands = [];
    try {
      ans = evalExp(n.children[0], fr);
    }
    catch (e) {
      if (!(e instanceof StackCleaner)) console.trace();
      throw e;      
...
function evalFun(fn, args, withNew, cn) {
  ...
  while(true) {
    try {
      ...
      if (result && (!aveq(result[0], retval) ||
                     !aveq(result[1], errval))) {
        // The result changed the last summary; update summary and keep going.
        addSummary(fn, args,
                   avjoin(result[0], retval), avjoin(result[1], errval),
                   tsAtStart);
      }
      else {
...
function evalToplevel(tl) {
  ...
  while (w.length !== 0) {
    ...
    if (n.type === RETURN)
      ...
      ans = evalStm(n, fr);
      if (ans === undefined) print("etl " + n.lineno);
      fr = ans.fr;
      w.push(ans.v);
...
// Infer types for all objects and store them in infTypes
function inferTypes() {
  var x;
  for (var i = 0; i < count; i++) {
    x = heap[i];
    if (x instanceof Aobj &&
        infTypes[i] !== undefined)
      objToType(x, i);
  }
}
